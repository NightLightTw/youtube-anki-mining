...
| **繁中釋義整批留空**，畫面出現 `中文翻譯失敗：HTTP Error 429` | 中文靠的是 Google 翻譯網頁版的內部端點（非官方、沒有保證），Google 隨時可能收緊某個存取管道。**這通常與你的網路或 IP 無關**——換 VPN 多半沒用。管線內建了備援管道會自動輪替；若全部失效，可先照常建卡再手動補中文欄。 |
...
1. **詞義消歧不完美** — 一個字在字典常有多個詞條/義項，管線靠詞性猜測＋內容重疊評分挑義項，遇到歧義結構或字典缺少該詞性詞條時會選錯，卡片會顯示與句意不符的定義。
   管線挑不出依據時會**在卡片打上 tag**（卡面不變，複習時看不到），你可以在 Anki 搜尋列篩出來複查：
   - `tag:pos-uncertain` — 字典裡有多種詞性，但句子看不出是哪一種，只能用排第一的。整條定義的詞性都可能不對，**這類最值得優先看**。
   - `tag:sense-uncertain` — 詞性對了，但同一詞性下的幾個意思沒有一個對得上例句。噪音較多。
   - `tag:no-definition` — 字典查不到這個字，Definition 欄是空的。
   - `tag:definition-lookup-failed` — 查詢當下失敗了（網路、逾時、額度用盡），不是字典沒收。重跑同一支影片就會補上。
   注意**沒被標記不代表就是對的**：管線「有把握地選錯」的情況抓不到（實測樣本裡約每 3 個錯誤會漏掉 1 個）。
...
實務緩解：建卡後在 Anki 掃一遍，刪掉或修正不對的卡；上面那兩個 tag 可以當作優先檢查的名單。系統性解法見下方 Roadmap 的 LLM 輔助方向。

### 音檔可能漏掉句首的幾個字

如果某張卡的音檔聽起來像從半句開始，多半是這個原因。

管線要知道「這句話在影片的第幾秒到第幾秒」才能切音檔。YouTube 自動聽打的字幕會附每個字的精確時間，直接用就很準；但**頻道自己上傳的人工字幕沒有逐字時間**，只能拿整段字幕的時間去推算。上字幕的人若習慣性地晚標幾格，整支影片的切點就跟著偏，音檔開頭就會少掉一兩個字。

實測四支 BBC podcast，句首偏移的中位數從 −0.12 秒（安全）到 +0.24 秒（會漏字）都有，**同一個頻道的不同節目差異就很大**。

自動字幕影片的切點準得多，但**不是完全免疫**：管線原本會在算出時間後再做一次「靜音吸附」（把邊界對齊到最近的停頓），實測那一步反而會把已經夠準的邊界推移零點幾秒，正好是一到三個字的長度。現在有逐字時間的句子已改為跳過這一步；如果你的素材反而變糟，可以退回舊行為：
...
- **換一句**——同一個字通常在影片裡出現不只一次，用 `--list` 找別的索引重做
- **用手動模式微調**——`mine.py --index N --word WORD` 自己挑句子
- 如果偏移**很大**（幾秒等級，整份字幕明顯錯位），可以先用 [ffsubsync](https://github.com/smacke/ffsubsync) 之類的字幕同步工具修好 SRT 再製卡。但要注意它是為「秒」等級的錯位設計的，實測對本專案這種零點幾秒的偏移**反而會改壞**，不要拿來處理細微偏移

> 這個現象調查過一輪，結論是目前沒有划算的自動解法：現成的字幕同步工具解析度不夠，強制對齊類的工具（torchaudio、WhisperX、NeMo）準確但都要裝 PyTorch。細節與量測數據見 [issue #1](https://github.com/NightLightTw/youtube-anki-mining/issues/1)；`tools/asr_timing.py` 是給進階使用者的選用工具（需自備 whisper 環境）。
...
| `tools/bench_cut.py` | 量測音檔切點誤差的工具（開發用）。拿有逐字時間戳的影片當標準答案，量出「只能靠推算」那條路徑的誤差，用來驗證參數調整有沒有效 |
| `tools/asr_timing.py` | 選用：用本機語音辨識產生逐字時間戳，給人工字幕影片用。需自備 whisper 環境，一般使用者不需要 |
