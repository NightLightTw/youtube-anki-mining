...
1. **詞義消歧不完美** — 一個字在字典常有多個詞條/義項，管線靠詞性猜測＋內容重疊評分挑義項，遇到歧義結構或字典缺少該詞性詞條時會選錯，卡片會顯示與句意不符的定義。建卡時若所有候選義項都對不上例句，管線會標記該字並在跑完後列出，提醒你回頭確認——但那只是提醒，抓不到「有分數卻選錯」的情況。
...
實務緩解：建卡後在 Anki 掃一遍，刪掉或修正不對的卡；跑完時列出的「詞義可能挑錯」清單可以當作優先檢查的名單。系統性解法見下方 Roadmap 的 LLM 輔助方向。
