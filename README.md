...
> 自動挑字品質不會比手動好——它靠頻率＋已知字庫猜，建完在 Anki 裡掃一遍刪掉不要的即可。詳見「[已知限制](#known-limitations)」。
...
1. **詞義消歧（word-sense disambiguation）不完美** — 一個字在 MW 常有多個詞條/義項，管線靠詞性猜測（`_guess_pos`）＋定義與句子的內容重疊評分挑義項，但遇到「限定詞 + 名詞/形容詞」歧義（如 *the keenest eye*）、MW 缺少該詞性詞條（如 *jarring* 只有名詞/動詞 *jar*）等情況仍會選錯，卡片會顯示與句意不符的定義。
2. **詞形還原（lemmatization）有錯漏** — `simplemma` 會把部分規則變化還原壞（如 *gone* → 錯誤結果），目前靠 `LEMMA_OVERRIDES` 逐字修補，屬於打地鼠式的維護。
3. **MW 學習者字典收錄有缺口** — 現代口語/慣用義常查不到（*dupe*=仿冒品、*jam*=最愛的事物、*chill*=放鬆的），英式拼法需要備援轉換（`-ise`→`-ize`、`traveller`→`traveler`），少數字完全沒有詞條（*existential*）。
4. **中文釋義偶爾對不上英文定義** — Google 翻譯即使有英文定義當語境提示，仍可能給出字面直翻或語域錯誤的翻譯。
5. **自動字幕品質限制** — 逐字稿的口吃斷詞（*ch-*）、連字號黏字（*over-looming*）、拼寫數字（*fifty-nine*）已有過濾器擋掉常見型態，但無法窮舉；完全沒有標點的自動字幕（部分影片）則無法斷句，目前直接不支援。

實務上的緩解方式：建卡後在 Anki 掃一遍，刪掉或修正不對的卡（也是目前作者的實際工作流——搭配 LLM 逐卡審查修正，見下方 Roadmap）。
...
- **詞義消歧**：把整句 + MW 的候選義項丟給 LLM 挑最符合句意的一個，取代目前的詞性啟發式 + 重疊評分（限制 1 的根治方案）。
- **挑字品質**：讓 LLM 判斷候選字在該句中是否值得學（過濾專有名詞誤判、無學習價值的組合詞），取代部分頻率規則。
- **釋義補洞**：MW 查無詞條或義項時，讓 LLM 直接產生該句境下的英英定義 + 繁中釋義（限制 3、4 的備援）。
- **中譯校對**：讓 LLM 核對 Google 翻譯結果與英文定義是否一致，不一致時重寫。

實作上可設計成**選用層**（如 `--llm-assist` 旗標 + API 金鑰），沒設定時退回現行純規則管線，維持零額外成本可用。目前此流程由作者在 Claude Code 對話中人工執行（跑完管線後逐卡審查、查證 MW、修正欄位），已驗證有效，待內建進 `mine.py`。
