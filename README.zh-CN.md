...
1. **词义消歧不完美** — 一个字在字典常有多个词条/义项，管线靠词性猜测＋内容重叠评分挑义项，遇到歧义结构或字典缺少该词性词条时会选错，卡片会显示与句意不符的定义。
   管线挑不出依据时会**在卡片打上 tag**（卡面不变，复习时看不到），你可以在 Anki 搜索列筛出来复查：
   - `tag:pos-uncertain` — 字典里有多种词性，但句子看不出是哪一种，只能用排第一的。整条定义的词性都可能不对，**这类最值得优先看**。
   - `tag:sense-uncertain` — 词性对了，但同一词性下的几个意思没有一个对得上例句。噪音较多。
   - `tag:no-definition` — 字典查不到这个字，Definition 栏是空的。
   - `tag:definition-lookup-failed` — 查找当下失败了（网络、逾时、额度用尽），不是字典没收。重跑同一支视频就会补上。
...
实务缓解：建卡后在 Anki 扫一遍，删掉或修正不对的卡；上面那两个 tag 可以当作优先检查的名单。系统性解法见下方 Roadmap 的 LLM 辅助方向。
